...
def args_main(*args, **kwargs):
    ...
    # # GPU 设备选择
    # parser.add_argument("--gpu-id", type=int, default=0, help="指定要使用的 GPU 设备编号")

    # 时间步长 (用于 SNN 或 Transformer)
    parser.add_argument("--T", type=int, default=10, help="时间步长")

    # 选择学习率调度器
    parser.add_argument("--scheduler", type=str, default="cosine", choices=["cosine", "reduce"],
                        help="选择学习率调度器: cosine (CosineAnnealing) 或 reduce (ReduceLROnPlateau)")
    parser.add_argument("--eta_min", type=float, default=0.0001, help="CosineAnnealing 的最小学习率")
    parser.add_argument("--use_bn", type=bool, default=True, help="是否使用 BatchNorm")
    parser.add_argument("--use_dropout", type=bool, default=True, help="是否使用 Dropout")
