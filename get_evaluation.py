...
def get_evaluation():
    ...
    if args.AVCA==True:
        model_params = get_model_params(config.lr, config.first_additional_triplet, config.second_additional_triplet, \
                                        ...
                                        config.T, config.scheduler, config.eta_min, config.epochs, config.use_bn, config.use_dropout)
    ...
    # print(f"Loading model A from {weights_path_stage_A} with epoch {epoch_A}")
    for epoch_A in range(50):
        weights_path_stage_B = list((args.load_path_stage_B / "checkpoints").glob(f"*_ckpt_{epoch_A}.pt"))[0]
